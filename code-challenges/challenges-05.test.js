...
const listFoods = (recipe) => {
  let result = [];
  let result2=[];
  for(let val of recipe.ingredients) {
    result2.push( val.replace(/\d|pounds|gallons|medium-sized|pound|cups|''/gi, ' ').trim());
  }
...
const stepActions = (recipe) => {
  let result = [];
  // Solution code here...
  for(let val of recipe.steps) {
    result.push(val.split(' ').splice(0,1).join(''));
  }
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  // Solution code here...
  let newA = str.split('');
  newA.splice(- numberOfCharacters);
  return newA.join('');
};
