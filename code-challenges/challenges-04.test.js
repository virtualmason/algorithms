...
const citiesAtoJ = (arr) => {
  ...
  arr.forEach(function(item){
    let regx = /[A-J]/;
    let answer = regx.test(item[0]);
    answer?capitals.push(item):'';
  });
  return capitals;
};
/* ------------------------------------------------------------------------------------------------
...
const matchMonth = (input) => {
  var arr = ['October', 'Oct', 'october', 'oct'];
  return arr.includes(input)? input: "";
};
